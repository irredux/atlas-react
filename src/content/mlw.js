...
function ExternalConnectionAuthorRow(props){
    ...
    return <><td style={{color: props.cEl.in_use!==1?"lightgray":"inherit"}} title={"ID: "+props.cEl.id}>{props.cEl.in_use!==1?"[":null}<aut>{props.cEl.abbr}</aut> <small><span dangerouslySetInnerHTML={parseHTML(props.cEl.full)}></span> (<span dangerouslySetInnerHTML={parseHTML(props.cEl.date_display)}></span>)</small>{props.cEl.in_use!==1?"]":null}{works.length>0?<ul style={{listStyleType: "none"}}>{works.map(w=><li><span dangerouslySetInnerHTML={parseHTML(w.opus)}></span> <small>(<span dangerouslySetInnerHTML={parseHTML(w.date_display)}></span>)</small></li>)}</ul>:<small style={{marginLeft: "10px", cursor: "pointer", color: "#0d6efd"}} onClick={async()=>{setWorks(await arachne.work.get({author_id: props.cEl.id}, {select: ["id", "opus", "date_display"]}))}}>Werke laden...</small>}</td>
