...
function ExternalConnectionAuthorRow(props){
    ...
    return <><td style={{color: props.cEl.in_use!==1?"lightgray":"inherit"}} title={"ID: "+props.cEl.id}>{props.cEl.in_use!==1?"[":null}<aut>{props.cEl.abbr}</aut> <small><span dangerouslySetInnerHTML={parseHTML(props.cEl.full)}></span>{props.cEl.date_display?<> (<span dangerouslySetInnerHTML={parseHTML(props.cEl.date_display)}></span>)</>:null}</small>{props.cEl.in_use!==1?"]":null}{works.length>0?<ul style={{listStyleType: "none"}}>{works.map(w=><li><span dangerouslySetInnerHTML={parseHTML(w.opus)}></span> <small>(<span dangerouslySetInnerHTML={parseHTML(w.date_display)}></span>)</small></li>)}</ul>:<small style={{marginLeft: "10px", cursor: "pointer", color: "darkgray"}} onClick={async()=>{setWorks(await arachne.work.get({author_id: props.cEl.id}, {select: ["id", "opus", "date_display"]}))}}><br />Werke laden...</small>}</td>
...
function ExternalConnectionWorkRow(props){
    ...
    useEffect(()=>{
        const fetchData=async()=>{
            const newAuthor = await arachne.author.get({id: props.cEl.author_id}, {select: ["gq_id", "cc_idno", "mirabile_id"]});
