...
function ExternalConnectionAuthorRow(props){
    ...
    return <><td style={{color: props.cEl.in_use!==1?"lightgray":"inherit"}} title={"ID: "+props.cEl.id}>{props.cEl.in_use!==1?"[":null}<aut>{props.cEl.abbr}</aut> <small dangerouslySetInnerHTML={parseHTML(props.cEl.full)}></small>{props.cEl.in_use!==1?"]":null}<br /><span dangerouslySetInnerHTML={parseHTML(props.cEl.date_display)}></span>{works.length>0?<ul>{works.map(w=><li><span dangerouslySetInnerHTML={parseHTML(w.opus)}></span></li>)}</ul>:<small style={{marginLeft: "10px", cursor: "pointer"}} onClick=async(){setWorks(await arachne.work.get({author_id: props.cEl.id}, {select: ["id", "opus", "date_display"]}))}>Werke laden...</small>}</td>
