...
function ExternalConnectionAuthorRow(props){
    ...
    return <><td style={{color: props.cEl.in_use!==1?"lightgray":"inherit"}} title={"ID: "+props.cEl.id}>{props.cEl.in_use!==1?"[":null}<aut>{props.cEl.abbr}</aut> <small dangerouslySetInnerHTML={parseHTML(props.cEl.full)}></small>{props.cEl.in_use!==1?"]":null}<br /><small dangerouslySetInnerHTML={parseHTML(props.cEl.date_display)}></small>{works.length>0?<ul>{works.map(w=><li dangerouslySetInnerHTML={parseHTML(w.opus)}></li>)}</ul>:<small style={{marginLeft: "10px"}} onClick={async()=>{setWorks(await arachne.work.get({author_id: props.cEl.id}, {select: ["id", "opus"]}))}}>Werke laden...</small>}</td>
